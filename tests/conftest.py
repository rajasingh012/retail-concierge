...
can flag off-vocabulary product_type / brand values. When a test
exercises the production code path that calls ``_seed_brief_validator``
(for example via ``build_shopping_agent``), vocabulary persists into
the next test that does not explicitly reset it. That bleeds into
...
def _reset_catalog_vocabulary() -> None:
    ...
    We call ``set_catalog_vocabulary(set(), set())`` once before
    yielding rather than using a yield/cleanup pair, because the
    goal is to start each test from the SAME known-good baseline
    rather than to restore whatever the test produced. Tests that
    intentionally seed (e.g. via ``build_shopping_agent`` then
    ``model_validate``) end with their own seeded state regardless
    — the next test re-clears, which is what we want.
    """
